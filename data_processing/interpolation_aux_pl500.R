...
nc <- nc_open(paste0(data_dir, "forecasts/auxiliary/ecmwf_aux_pl500_2007.nc"))
nc
...
fc_interpolated_v <- fc_interpolated_gh <- fc_interpolated_u
...
for(thisst in stationlist){
  ...
  for(vtime in fc_raw_validtime){
    ...
    # gh
    fc_interpolated_gh[thisst_pos, , vtime_pos] <- fc_raw_gh[lon_pos_mindiff, lat_pos_mindiff, , vtime_pos]
  }
}
...
other_files <- paste0(data_dir, "forecasts/auxiliary/ecmwf_aux_pl500_", 2008:2016, ".nc")
...
for(filename in other_files){
  ...
  fc_interpolated_v_append <- fc_interpolated_gh_append <- fc_interpolated_u_append
  ...
  for(thisst in stationlist){
    ...
    for(vtime in fc_raw_validtime_append){
      ...
      # gh
      fc_interpolated_gh_append[thisst_pos, , vtime_pos] <- fc_raw_gh[lon_pos_mindiff, lat_pos_mindiff, , vtime_pos]
    }
  }
  ...
  fc_interpolated_v <- abind(fc_interpolated_v,
                             ...
                             along = 3)
  fc_interpolated_gh <- abind(fc_interpolated_gh,
                              fc_interpolated_gh_append,
                              along = 3)
  fc_raw_validtime <- c(fc_raw_validtime, fc_raw_validtime_append)
  rm(fc_interpolated_u_append, fc_interpolated_v_append, fc_interpolated_gh_append)
}
...
## create nc file
ncfile_name <- paste0(data_dir,"data_aux_pl500_interpolated.nc")
...
ncvar_put(nc = ncout, varid = gh_fc_def, vals = fc_interpolated_gh)
