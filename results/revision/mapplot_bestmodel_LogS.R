...
load("/media/sebastian/Elements/Postproc_NN/LogS_all.Rdata")
...
df_stat_15[,5:14] <- NA
for(j in 5:14){
  names(df_stat_15)[j] <- names(df_logs_15)[j-1]
}
df_stat_15[,15] <- NA
...
for(j in 5:14){
  names(df_stat_0715)[j] <- names(df_logs_0715)[j-1]
}
df_stat_0715[,15] <- NA
...
for(i in 1:length(df_stat_15$station_id)){
  ...
  # 2015 pp models
  df_st_pp_15 <- subset(df_logs_15, station == st)
  ...
  # 2007-2015 pp models
  df_st_pp_0715 <- subset(df_logs_0715, station == st)
  df_st_pp_0715_mean <- apply(df_st_pp_0715[,4:ncol(df_st_pp_0715)], 2, mean)
  df_stat_0715[i,5:13] <- df_st_pp_0715_mean
  
  # Ensemble
  # df_st_ens <- subset(df_res_ens, station == st)
  # df_stat_15[i,15] <- df_stat_0715[i,15] <- mean(df_st_ens$logs)
}
...
for(i in 1:length(df_best_15$station_id)){
  ...
  # 2015
  df_st <- subset(df_logs_15, station == st)
  if(nrow(df_st) == 0){
    next
  }
  ...
  # 2007-15
  df_st <- subset(df_logs_0715, station == st)
  df_st_means <- apply(df_st[,4:ncol(df_st)], 2, mean)
  best <- names(df_st_means)[which(df_st_means == sort(df_st_means)[1])]
  second <- names(df_st_means)[which(df_st_means == sort(df_st_means)[2])]
  diff <- df_st_means[best] - df_st_means[second]
...
allmodels_0715 <- names(df_logs_0715)[4:ncol(df_logs_0715)]
allmodels_15 <- names(df_logs_15)[4:ncol(df_logs_15)]
mypal <- rainbow_hcl(length(allmodels_0715)+2)
...
pdf("mapplot_bestmodel_rev_LogS.pdf", width = 10, height = 6, pointsize = 12)
