...
fit_qrf <- function(ntree, nodesize, mtry, replace){
  ...
  Routname1 <- paste0("alldata_qrf_local_ntree", ntree, "_nodesize", nodesize, "_mtry", mtry, "_repl", replace, ".Rout")
  ...
  savename <- paste0("alldata_qrf_local_ntree", ntree, "_nodesize", nodesize, "_mtry", mtry, "_repl", replace, ".Rdata")
  fname <- paste0("...", savename)
  
  save(qrf_crps, file = fname)
  
  print(mean(qrf_crps, na.rm = TRUE))
