## load data (see crch_global_boosting_test)
...
traindates_vec <- rep(dates[ind_training], each = 537)[ind_notNA]
trainstations_vec <- rep(stations, length(ind_training))[ind_notNA]
...
## load evaluation data (required to compute oos parameters in loop over stations)
...
evaldates_vec <- rep(dates[ind_eval], each = 537)[ind_notNA]
evalstations_vec <- rep(stations, length(ind_eval))[ind_notNA]
...
loc_save <- numeric(nrow(data_eval))
sc_save <- numeric(nrow(data_eval))
...
for(this_station in stations){
  ...
  ind_this_station <- which(trainstations_vec == this_station)
  ...
  crch_model <- crch(obs ~ ens_mean  | ens_var,
                     data = data_train[ind_this_station,],
                     dist = "gaussian",
                     link.scale = "log")
  
  # compute parameters on evaluation data
  
  ind_this_station_eval <- which(evalstations_vec == this_station)
  
  loc <- as.numeric(predict(crch_model, 
                            data_eval[ind_this_station_eval,], 
                            type = "location"))
  sc <- as.numeric(predict(crch_model, 
                           data_eval[ind_this_station_eval,], 
                           type = "scale"))
  
  # save parameters
  
  loc_save[ind_this_station_eval] <- loc 
  sc_save[ind_this_station_eval] <- sc
}

mean(crps_norm(y = data_eval$obs, mean = loc_save, sd = sc_save)) # 0.9310598


## full model with boosting and all variables
...
for(this_station in stations){
  ...
  # omit stations with too few observation to successfully estimate model
  # will appear as NA in loc_save/sc_save if this station occurs in eval data 
  if(length(ind_this_station) <= 10){next}
  
  # estimate model on training data
  
  crch_model <- crch(obs ~ .|.,
                     data = data_train[ind_this_station,1:35],
                     dist = "gaussian",
                     link.scale = "log",
                     method = "boosting",
                     mstop = "aic",
...
mean(crps_norm(y = data_eval$obs, mean = loc_save, sd = sc_save)) # 0.8901908

oos_mean <- loc_save
oos_sd <- sc_save
oos_dates <- as.character(evaldates_vec)
oos_stations <- evalstations_vec

df <- data.frame(cbind(oos_dates, oos_mean, oos_stations, oos_sd))
names(df) <- c("date", "mean", "station_id", "std")

write.csv(df, file = "results_emos_boosting_local.csv", quote = FALSE)

## more results
# aic, maxit 500: 0.8901908
# bic, 500: 0.8917257
# cv, 500: 0.88838
