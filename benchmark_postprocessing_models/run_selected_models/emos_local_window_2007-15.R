...
## training 2007-15
## EMOS local, "climatological" window: for day d0, use [d0 - m days, d0 + m days] in all previous years
...
postproc_local <- function(vdate, window_width_half){
  ...
  data_train_dates <- data[which(is.element(as.Date(data$date), as.Date(train_dates))), ]
...
for(day_id in 1:length(eval_dates)){
  ...
  # post-processing
  par_out <- postproc_local(vdate = today, window_width_half = m)
...
emos_local_window_alldata <- df_out
save(emos_local_window_alldata,
     file = "/home/sebastian/Projects/PP_NN/code/results/Rdata_files/emos_local_window_2007-15.Rdata")
