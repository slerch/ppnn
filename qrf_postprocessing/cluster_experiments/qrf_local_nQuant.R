...
## here: assess effect of number of quantiles used in qt_levels
...
fit_qrf <- function(ntree, nodesize, mtry, replace, nquant){
  
  # quantile levels
  nn <- nquant + 1
  qt_levels <- seq(1/nn, (nn-1)/nn, by = 1/nn)
  qts_save <- matrix(NA, 
                     nrow = nrow(data_eval_all), 
                     ncol = length(qt_levels))
  
  Routname1 <- paste0("qrf_local_ntree", ntree, "_nodesize", nodesize, "_mtry", mtry, "_repl", replace, "_nQuant", nquant, ".Rout")
  ...
  savename <- paste0("qrf_local_ntree", ntree, "_nodesize", nodesize, "_mtry", mtry, "_repl", replace, "_nQuant", nquant, ".Rdata")
...
#     default: 1 (or 0)
# nquant: number of quantiles (default from before: 50)
...
pars <- expand.grid(ntree.try, nodesize.try, mtry.try, replace.try, nquants.try)

nrow(pars) # --> 72 tasks

ID <- as.numeric(as.character(Sys.getenv(c("SGE_TASK_ID"))))

fit_qrf(ntree = pars[ID,1], nodesize = pars[ID,2], mtry = pars[ID,3], replace = pars[ID,4], nquant = pars[ID, 5])
