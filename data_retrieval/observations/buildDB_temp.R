...
# database file
dbPath <-
  paste0(myPath, "data_files/station_data")


#### build database ####

# temperature filename structure "produkt_tu_stunde_'START'_'END'_'ID'.txt"
# START 19:26
# END 28:35
# ID 37:41
...
tempCsvList <- dir(dataPath, pattern = "produkt_tu_stunde_") %>%
...
db <- dbConnect(SQLite(), dbPath)
try({
  IDs <- dbGetQuery(db, 'SELECT STATIONS_ID FROM temp') %>%
    .$STATIONS_ID %>%
    unique
  
  read_fwf(
    str_c(dataPath, "TU_STUNDENWERTE_BESCHREIBUNG_STATIONEN.txt"),
    fwf_positions(
      c(1, 25, 40, 52, 62),
      c(5, 38, 50, 60, 101),
