...
#   print(ind_st)
#   data_eval <- subset(data, date == tt & station == this_station)
...
#   sc[ind_st] <- sc_st
# }
# data_eval_all <- subset(data, date == tt)
# summary(crps_norm(y = data_eval_all$obs, mean = loc, sd = sc))
...
for(day_id in 1:length(eval_dates)){
  ...
  for(this_station in stations_list){
    ind_st <- which(stations_list == this_station)
    # print(ind_st)
    data_eval <- subset(data_eval_all, date == tt & station == this_station)
    if(!is.finite(data_eval$obs)){next}
    loc_st <- c(cbind(1, data_eval$t2m_mean) %*% par_out[ind_st,1:2])
    scsquared_tmp <- c(cbind(1, data_eval$t2m_var) %*% par_out[ind_st,3:4])
    if(scsquared_tmp <= 0){
